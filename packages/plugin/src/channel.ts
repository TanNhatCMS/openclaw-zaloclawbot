...
export function clampClawbotChunkLimit(limit: number | undefined): number {
  const value = typeof limit === "number" && limit > 0 ? limit : DEFAULT_TEXT_CHUNK_LIMIT;
  return Math.min(value, ZALO_MESSAGE_HARD_LIMIT);
}

function normalizeClawbotMessagingTarget(raw: string | null | undefined): string | undefined {
...
function looksLikeClawbotTargetId(
  raw: string | null | undefined,
  normalized: string | null | undefined = raw,
): boolean {
  const value =
    normalizeClawbotMessagingTarget(normalized) ?? normalizeClawbotMessagingTarget(raw);
...
export const clawbotPlugin: ChannelPlugin<ResolvedClawbotAccount> = {
  id: CHANNEL_ID,
  meta: {
    id: CHANNEL_ID,
    label: "Zalo ClawBot",
  ...
  setupWizard: {
    channel: CHANNEL_ID,
    getStatus: async (ctx) =>
      (await import("./setup/onboarding.js")).clawbotSetupWizard.getStatus(ctx),
    configure: async (ctx) =>
      (await import("./setup/onboarding.js")).clawbotSetupWizard.configure(ctx),
    configureInteractive: async (ctx) =>
      (await import("./setup/onboarding.js")).clawbotSetupWizard.configureInteractive!(ctx),
  },
    ...
    schema: {
      ...
      properties: {
        chunkMode: {
          ...
          enum: ["length", "newline"],
          description:
            "How to split long replies. 'length' (default): split only when a message exceeds textChunkLimit. 'newline': prefer paragraph boundaries for a more natural multi-message feel. Splitting is always markdown-safe.",
        },
        textChunkLimit: {
          type: "integer",
          minimum: 1,
          ...
          description:
            "Max characters per outgoing message (hard-capped at the platform's 2000-char limit; default 1990). Lower values chunk more aggressively; set to 2000 to keep a reply in one message whenever the platform allows.",
        },
      },
    },
  },
  ...
  outbound: {
    ...
    resolveEffectiveTextChunkLimit: ({ fallbackLimit }) =>
      clampClawbotChunkLimit(fallbackLimit),
    sendText: async (ctx) => {
      const account = resolveClawbotAccount(ctx.cfg, ctx.accountId);
      if (!account.botToken) {
        throw new Error(
          "clawbot not configured: run `openclaw channels login --channel openclaw-zaloclawbot`",
        );
      }
  ...
  auth: {
    login: async ({ cfg, accountId, runtime }) => {
      const sectionAccount = accountId
        ...
        : ({ accountId: "" } as ResolvedClawbotAccount);
      const sessionServiceUrl = sectionAccount.sessionServiceUrl;
      const log = (m: string) => runtime?.log?.(m);
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      if (!account.configured) {
        ...
        ctx.log?.error?.(
          `[${account.accountId}] clawbot not logged in — run: openclaw channels login --channel openclaw-zaloclawbot`,
        );
