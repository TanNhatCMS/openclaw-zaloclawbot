...
import { sendChatAction } from "../api/api.js";
...
export async function processOneClawbotMessage(update, deps) {
    ...
    const onReplyError = (err, info) => {
        aLog.error(`reply ${info.kind} error: ${String(err)}`);
    };
    const typing = {
        start: async () => {
            await sendChatAction(deps.botToken, {
                chat_id: ctx.To,
                action: "typing",
            }, undefined, ZALO_TYPING_TIMEOUT_MS);
        },
        onStartError: (err) => {
    ...
    const maybeDispatchReplyWithBufferedBlockDispatcher = compatRuntime.reply.dispatchReplyWithBufferedBlockDispatcher;
    if (typeof maybeRunAssembled === "function" &&
        typeof maybeDispatchReplyWithBufferedBlockDispatcher === "function") {
        const runAssembled = maybeRunAssembled;
        const dispatchReplyWithBufferedBlockDispatcher = maybeDispatchReplyWithBufferedBlockDispatcher;
        await runAssembled({
            cfg: deps.config,
            channel: "openclaw-zaloclawbot",
            accountId: deps.accountId,
            ...
            record: {
                ...
                onRecordError: (err) => aLog.error(`recordInboundSession: ${String(err)}`),
            },
        });
        return;
    }
