import { ZaloApiError, deleteWebhook, getUpdates, getWebhookInfo, } from "../api/api.js";
...
export async function startClawbotProvider(opts) {
    const { account, config, abortSignal, runtime, channelRuntime, setStatus } = opts;
    const aLog = logger.withAccount(account.accountId);
    const log = runtime?.log ?? (() => { });
    const errLog = runtime?.error ?? ((m) => log(m));
    if (!account.botToken) {
        aLog.error("startClawbotProvider: not configured (no botToken)");
        errLog(`[${account.accountId}] zaloclawbot not logged in — run: openclaw channels login --channel openclaw-zaloclawbot`);
    ...
    const enqueueUpdate = (update) => {
        processingTail = processingTail
            ...
            .then(() => processOneClawbotMessage(update, {
            accountId: account.accountId,
            botToken: token,
            config,
    ...
    while (!abortSignal.aborted) {
        try {
            const resp = await getUpdates(token, { timeout: DEFAULT_LONG_POLL_SECONDS }, undefined, abortSignal);
            ...
            aLog.error(`getUpdates failed (${consecutiveFailures}/${MAX_CONSECUTIVE_FAILURES}): ${String(err)}`);
            errLog(`[${account.accountId}] getUpdates failed: ${String(err)}`);
            const backoffMs = consecutiveFailures >= MAX_CONSECUTIVE_FAILURES ? LONG_BACKOFF_MS : ERROR_BACKOFF_MS;
...
async function clearStaleWebhook(token, accountId, log) {
