...
import type { ChannelSetupWizardAdapter, WizardPrompter } from "openclaw/plugin-sdk/setup";
...
function hasConfiguredClawbotAccount(): boolean {
  return listIndexedClawbotAccountIds().some((id) =>
    Boolean(loadClawbotAccount(id)?.botToken?.trim()),
  );
}

function resolveSessionServiceUrl(cfg: OpenClawConfig): string | undefined {
...
function enableClawbotChannel(cfg: OpenClawConfig): OpenClawConfig {
  const section = cfg.channels?.[CHANNEL_ID] ?? {};
  return {
    ...cfg,
    channels: {
 ...
 */
async function runClawbotQrOnboarding(cfg: OpenClawConfig, prompter: WizardPrompter) {
  const result = await runClawbotQrLoginLoop({
    ...
    ui: {
      renderQr: async (qr, loginUrl, attempt, max) => {
        ...
        await prompter.note(
          "Scan the QR below with the Zalo app to connect your bot.",
          title,
        );
...
export const clawbotSetupWizard: ChannelSetupWizardAdapter = {
  channel: CHANNEL_ID,
  ...
  configure: async ({ cfg, prompter }) =>
    (await runClawbotQrOnboarding(cfg, prompter)) ?? { cfg },

  configureInteractive: async ({ cfg, prompter }) =>
    (await runClawbotQrOnboarding(cfg, prompter)) ?? "skip",
};