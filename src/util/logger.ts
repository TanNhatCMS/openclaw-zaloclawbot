...
function resolveMinLevel(): number {
  ...
  if (env && env in LEVEL_IDS) return LEVEL_IDS[env as LogLevelName];
...
export function setLogLevel(level: string): void {
  ...
  minLevelId = LEVEL_IDS[upper as LogLevelName];
}
...
function writeLog(level: LogLevelName, message: string, accountId?: string): void {
...
export interface ClawbotLogger {
  info: (message: string) => void;
  debug: (message: string) => void;
  warn: (message: string) => void;
  error: (message: string) => void;
  withAccount: (accountId: string) => ClawbotLogger;
  getLogFilePath: () => string;
}

function createLogger(accountId?: string): ClawbotLogger {
  return {
