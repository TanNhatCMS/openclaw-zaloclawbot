...
export async function sendClawbotText(params: {
  ...
  if (!to) throw new Error("clawbot send: chat_id is required");
  if (!token) throw new Error("clawbot send: bot token is required");

  const res = await sendMessage(token, { chat_id: to, text, parse_mode: PARSE_MODE });
  const messageId = String(res.result?.message_id ?? "");
  logger.info(
    `outbound: to=${to} textLen=${text.length} messageId=${messageId || "(none)"}`,
  );
  return { messageId };
}