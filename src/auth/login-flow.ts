...
export interface ClawbotLoginUi {
  renderQr: (
    qr: string | null,
    loginUrl: string,
    attempt: number,
  ...
  beginWait: (message: string) => { stop: (message: string) => void };
}
 ...
 */
export async function renderClawbotLoginQr(loginUrl: string): Promise<string | null> {
  try {
...
export async function persistClawbotLogin(wait: {
  ...
  if (identity.usedTokenFallback) {
    logger.warn(
      `persistClawbotLogin: using token-derived account id because getMe did not resolve botId tokenPublicId=${String(identity.tokenPublicId)}`,
    );
  }
...
export async function runClawbotQrLoginLoop(params: {
  ...
  reloadConfigAfterLogin?: boolean;
  ...
  for (let attempt = 1; attempt <= MAX_QR_REFRESH; attempt++) {
    ...
    if (attempt < MAX_QR_REFRESH) {
      waiter.stop(
        `${wait.message} Generating a new QR (${attempt + 1}/${MAX_QR_REFRESH})…`,
      );
      continue;
    }
 ...
 * the login itself.
 */
export async function triggerClawbotChannelReload(): Promise<void> {
  try {
